...
import { Clock, BarChart2, Plus, Search, Sparkles } from 'lucide-react'
          ...
          if (tab.primary) {
            return (
              <Link key={tab.href} href={tab.href}>
                <motion.div
                  whileTap={{ scale: 0.9 }}
                  className="w-14 h-14 rounded-2xl bg-gradient-to-br from-violet-600 to-blue-500 flex items-center justify-center shadow-lg shadow-violet-300/50"
                >
          ...
          return (
            <Link key={tab.href} href={tab.href} className="flex flex-col items-center gap-0.5 px-3 py-1 min-w-[52px]">
              <motion.div whileTap={{ scale: 0.9 }} className="flex flex-col items-center gap-0.5">
                ...
                />
                <span className={`text-[10px] font-semibold transition-colors ${isActive ? 'text-violet-600' : 'text-gray-400'}`}>
                  {tab.label}
                </span>
              </motion.div>
            </Link>
